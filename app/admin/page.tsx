...
export default async function AdminHome({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  // Signed in — show analytics overview
  const admin = createAdminClient();
  const { data: orders } = await admin.from("orders").select("*");
  const { data: products } = await admin.from("products").select("*");
  const { data: items } = await admin.from("order_items").select("unit_price, unit_cost, quantity, order_id");

  const confirmed = (orders || []).filter((o: any) => o.status === "confirmed");
  const revenue = confirmed.reduce((s: number, o: any) => s + Number(o.total_amount), 0);
  const sst = confirmed.reduce((s: number, o: any) => s + Number(o.sst_amount), 0);
  const cogs = (items || [])
    .filter((it: any) => confirmed.some((o: any) => o.id === it.order_id))
    .reduce((s: number, it: any) => s + Number(it.unit_cost) * it.quantity, 0);
  const profit = revenue - cogs - sst;
  const profitPct = revenue ? ((profit / revenue) * 100).toFixed(1) : "0";
  const pending = (orders || []).filter((o: any) => o.status === "pending").length;

  const now = new Date();
  const thisMonth = confirmed.filter((o: any) => new Date(o.created_at).getMonth() === now.getMonth());
  const monthRevenue = thisMonth.reduce((s: number, o: any) => s + Number(o.total_amount), 0);

  const { data: settings } = await admin.from("invoice_settings").select("monthly_target").eq("id", 1).single();
